...
public class task01 {
    public static void main(String[] args) {
        ...
        int n = highestBit(i); // считаем номер старшего значащего бита выпавшего числа и сохраняем в n
    ...
    // метод для подсчета номера старшего значащего бита числа
    public static int highestBit(int num) {
        int bits = num == 0 ? 1 : 32 - Integer.numberOfLeadingZeros(num); // проверяем, не равно ли число 0, если равно,
                                                                          // возвращаем 1
        return bits;
    }
    ...
    public static int[] findNonMultiples(int n, int start, int end) {
        int[] m2 = new int[Math.abs(Short.MIN_VALUE - start) / n + Math.abs(end - Short.MIN_VALUE) / n + 1]; // вычисляем
                                                                                                             // размер
                                                                                                             // массива,
                                                                                                             // который
                                                                                                             // будем
